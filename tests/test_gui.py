...
def test_winning_a_battle_registers_the_keys(qapp, window, monkeypatch):
    ...
    assert window.recovered_rc2["22765AJ13F.pak"] == "DEADBEEF"   # keyed by name
...
def test_battle_victory_when_every_rom_cracks(qapp):
    from subaru_pak.gui.battle import BattleDialog
    dlg = BattleDialog(pak_path("22765AJ13F.pak"))
    dlg._on_finished({"rc2": "88295D8A",
                      "roms": [{"cid": "EB4I350A", "swap_key": (1, 2, 3, 4),
                                "swap_key_name": "denso_can"}]})
    assert "fainted" in dlg.message.text()
    assert dlg.run_btn.text() == "CLOSE"
    assert not dlg.fight_btn.isEnabled()
    dlg.close()


def test_battle_defeat_on_uncrackable_module(qapp):
    """A module whose core can't be cracked defeats the Graphics Card."""
    from subaru_pak.gui.battle import BattleDialog
    dlg = BattleDialog(pak_path("82201AL30D.pak"))
    got = {}
    dlg.recovered.connect(lambda s: got.update(s))
    dlg._on_finished({"rc2": "A9A12BB1",
                      "roms": [{"cid": "DF105742_SKE_REPchg2", "swap_key": None,
                                "swap_key_name": None}]})
    assert "worn out" in dlg.message.text()          # the card lost
    assert dlg.run_btn.text() == "CLOSE"
    assert got.get("rc2") == "A9A12BB1"              # RC2 still kept for next time
    dlg.close()


def test_recovered_keys_persist_across_restarts(qapp, settings, tmp_path):
    """Swap keys and per-pak RC2 keys survive closing and reopening the app."""
    from subaru_pak import keys
    keys.EXTRA_SWAP_KEYS.clear()
    win = gui.MainWindow(settings=settings)
    win.out_dir = str(tmp_path)
    pak = pak_path("22765AJ13F.pak")
    win._on_recovered(pak, {"rc2": "88295D8A",
                            "roms": [{"cid": "EB4I350A", "swap_key": (9, 8, 7, 6),
                                      "swap_key_name": "recovered_x"}]})
    win.close()
    keys.EXTRA_SWAP_KEYS.clear()                      # simulate a fresh process

    reopened = gui.MainWindow(settings=settings)      # same on-disk settings
    assert reopened.recovered_rc2.get("22765AJ13F.pak") == "88295D8A"
    assert any(k == (9, 8, 7, 6) for _, k in keys.all_swap_keys())
    reopened.close()
    keys.EXTRA_SWAP_KEYS.clear()
