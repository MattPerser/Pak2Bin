...
# The grind is narrated as a repeating turn: attack -> did it land -> outcome ->
# extra, one line per tick, a fresh random pick from each bucket each time.
_ATTACKS = [
    "{me} used BRUTE-FORCE!",
    "{me} is heating up!",
    "{me} is trying every key!",
    "{me} hurls keys at the lock!",
    "{me} spins up all {cores} cores!",
    "{me} unleashes a KEY BARRAGE!",
    "{me} used DICTIONARY SWEEP!",
]
_LANDS = [
    "A critical hit!",
    "It's a direct hit!",
...
_OUTCOMES = [
    ...
    "The {foe} PAK flinches!",
    "The {foe} PAK holds on!",
    "The {foe} PAK's guard drops!",
]
_EXTRAS = [
    "{foe} PAK used ENCRYPTION!",
    "{foe} PAK used 40-BIT RC2!",
    "{foe} PAK clings on with FF padding!",
    "{foe} PAK is unfazed!",
    "{foe} PAK hides behind the checksum!",
    "Fans screaming, keys pouring out!",
    "The keyspace narrows!",
]
_TURN = (_ATTACKS, _LANDS, _OUTCOMES, _EXTRAS)
...
class BattleDialog(QDialog):
    ...
    def _start_worker(self):
        ...
        self._taunt_timer.start(1500)
        self.run_btn.setText("RUN")
        self._thread.start()

    def _on_stage(self, note: str):
        self._line(f"{FIGHTER} is {note}!")
        self._turn_phase = 0                          # a real event restarts the beat
    ...
    def _taunt(self):
        bucket = _TURN[self._turn_phase % len(_TURN)]
        self._turn_phase += 1
        self._line(random.choice(bucket).format(
            me=FIGHTER, foe=self.foe_name, cores=os.cpu_count() or 1))
    ...
    def _on_finished(self, summary: dict):
        ...
        # Anything recovered is worth keeping (the RC2 key, plus any swap keys),
        # even on a partial or a loss -- it saves re-doing that work next time.
        self.summary = summary
        self.recovered.emit(summary)
        roms = summary.get("roms", [])
        cracked = [r for r in roms if r.get("swap_key")]
        locked = [r for r in roms if not r.get("swap_key")]

        if roms and not locked:
            self._victory(len(cracked), summary["rc2"])
        elif cracked:
            self._partial(len(cracked), len(locked), summary["rc2"])
        else:
            # Opened the archive but couldn't crack a single ROM -- a module
            # whose swap key isn't one of the known twelve and can't be
            # brute-forced from erased flash. The core held; the card loses.
            self._defeat(summary["rc2"])

    # -- battle outcomes --------------------------------------------------
    def _finish(self, foe_faints: bool, me_faints: bool):
        self.foe_hp.set_fraction(0.0 if foe_faints else 1.0)
        if foe_faints:
            self.foe_hp.set_hp_text("HP  0000 / 9999")
            self._faint(self.foe_sprite)
        if me_faints:
            self._faint(self.me_sprite)
    ...
    def _victory(self, n_keys: int, rc2: str):
        self._line(f"{self.foe_name} PAK fainted!\n"
                   f"{FIGHTER} recovered the key ({rc2}) and {n_keys} ROM key(s)!")
        self._finish(foe_faints=True, me_faints=False)

    def _partial(self, cracked: int, locked: int, rc2: str):
        self._line(f"{FIGHTER} cracked {cracked} ROM(s) — but {locked} core(s) "
                   f"held firm.\nKeys recovered so far are saved.")
        self._finish(foe_faints=False, me_faints=False)

    def _defeat(self, rc2: str):
        self._line(f"{FIGHTER} broke the shell (RC2: {rc2})… but the "
                   f"{self.foe_name} PAK's core is a module lock it can't pick!\n"
                   f"{FIGHTER} is worn out. The PAK stood its ground.")
        self._finish(foe_faints=False, me_faints=True)
