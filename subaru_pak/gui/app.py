...
class MainWindow(QMainWindow):
    ...
    def _restore_recovered_keys(self):
        """Re-register swap keys won in past sessions so detection finds them."""
    ...
    def _on_recovered(self, path, summary):
        ...
        if rc2_key and len(rc2_key.replace(" ", "")) == 8:
            self.recovered_rc2[path] = rc2_key.strip().upper()
            self.settings.setValue(f"rc2/{os.path.basename(path)}",
                                   self.recovered_rc2[path])
        ...
        if card:
            card.set_status("reading...")
            self._start(ScanJob(path, self._signals(), self.extension,
                                rc2_key=self.recovered_rc2.get(path)))
